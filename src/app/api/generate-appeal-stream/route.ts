...
/**
 * Get cached embeddings from DynamoDB
 */
  ...
  try {
    const result = await dynamoDbClient.send(
      new ScanCommand({
        ...
        ProjectionExpression: 'documentText, embedding',
      })
    );
    ...
    if (result.Items) {
      for (const item of result.Items) {
        if (item.documentText && item.embedding) {
          documentTexts.push(item.documentText as string);
          documentEmbeddings.push(item.embedding as number[]);
        }
      }
    }

    console.log(`📦 Found ${documentTexts.length} documents with embeddings`);
    return { documentTexts, documentEmbeddings };
  } catch (error) {
    console.error('Error fetching embeddings from DB:', error);
    return { documentTexts: [], documentEmbeddings: [] };
  }
}
